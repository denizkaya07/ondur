...
export default function Profil() {
  ...
  const sifreKaydet = async (e) => {
    ...
    setSifreHata('')
    setSifreBasari('')
    if (sifreForm.yeni !== sifreForm.yeni2) {
      setSifreHata('Yeni şifreler eşleşmiyor.')
      return
    }
    try {
      await api.post('/auth/sifre-degistir/', {
        eski_sifre: sifreForm.eski,
        yeni_sifre: sifreForm.yeni,
      })
...
const s = {
  kapsayici:   { padding: '2rem', maxWidth: '560px', margin: '0 auto' },
  baslik:      { fontSize: '1.5rem', fontWeight: '500', color: '#1a7a4a', marginBottom: '1.25rem' },
  ozetKart:    { display: 'flex', alignItems: 'center', gap: '16px', background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '1.25rem', marginBottom: '1.5rem' },
